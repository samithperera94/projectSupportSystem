...
/**
 * Servlet implementation class DoEditProjectProposal
 */
@WebServlet("/DoEditProjectProposal")
public class DoEditProjectProposal extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DoEditProjectProposal() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(errorString == null){
			try {
				ProjectProposalServices.editProjectProposal(conn, proposal);
				ProjectPlanServices.editProjectPlan(conn, plan);
	    ...
	    if (errorString != null) {
	    	RequestDispatcher dispatcher = request.getServletContext().getRequestDispatcher("/EditProjectProposal?success=0");
	        dispatcher.forward(request, response);
	       }
	    else {
